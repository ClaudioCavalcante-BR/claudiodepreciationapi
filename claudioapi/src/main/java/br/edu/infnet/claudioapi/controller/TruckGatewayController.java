...
public class TruckGatewayController {
	
	 private final TruckGatewayService truckGatewayservice;

	    public TruckGatewayController(TruckGatewayService truckGatewayservice) {
	        this.truckGatewayservice = truckGatewayservice;
	    }

	    // Proxy de consulta consolidada via claudiodepreciationapi
	    // GET http://localhost:8081/api/trucks/116/10602/2025-3
	    @GetMapping("/{marcaId}/{modeloId}/{anoCodigo}")
	    public ResponseEntity<TruckQueryResult>  consultar(
	            @PathVariable Integer marcaId,
	            @PathVariable Integer modeloId,
	            @PathVariable String anoCodigo) {
	    	
	    	TruckQueryResult result = truckGatewayservice.consultar(marcaId, modeloId, anoCodigo);
	    	// garante Diesel/D e mesReferencia 
